...
class Place(object):
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if not self.ant:
                self.ant = insect
            elif self.ant.can_contain(insect): # elif self.ant.is_container: and self.ant.can_contain(insect):
                self.ant.contain_ant(insect)
            elif self.ant and insect.is_container and insect.can_contain(self.ant):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            if not isinstance(insect, QueenAnt) or insect.queen_number > 1:
            # END Problem 13
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive, min_range = min_range, max_range = max_range):
        ...
        if self.place:
            current_location = self.place.entrance
        else:
            return None
        if len(self.place.bees) > 0 and min_range == 0:
            return random_or_none(self.place.bees)
        
        location_number = 0
        while current_location is not beehive:
            if len(current_location.bees) > 0 and location_number >= min_range and location_number < max_range:
    ...
    def action(self, colony):
        ...
        self.throw_at(self.nearest_bee(colony.beehive, min_range = self.min_range, max_range = self.max_range))
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if remaining_armor >0:
            ...
            for bee in bees_list:
                # print(bees_list[i])
                if not self.can_double_damage:
                    bee.reduce_armor(amount + (self.damage/2))
                else:
                    bee.reduce_armor(amount + self.damage)
        # print("DEBUG:", bees_list)
        Ant.reduce_armor(self, amount) # Insect.reduce_armor(self, amount)
...
class BodyguardAnt(Ant):
    ...
    def contain_ant(self, ant):
        ...
        if self.can_contain(ant):
            ...
            # self.contained_ant.can_double_damage = ant.can_double_damage
        # END Problem 9
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        Ant.how_many_queens += 1
        self.queen_number = Ant.how_many_queens
        ...
        if self.queen_number > 1:
            ...
            while current_place != None:
                ...
                if current_place.ant != None:
                    if current_place.ant.can_double_damage:
                        current_place.ant.damage *= 2
                        current_place.ant.can_double_damage = False
                    if isinstance(current_place.ant, BodyguardAnt) and current_place.ant.contained_ant != None and current_place.ant.contained_ant.can_double_damage:
                        current_place.ant.contained_ant.damage *= 2
                        current_place.ant.contained_ant.can_double_damage = False
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor == 0 and self.queen_number == 1:
            bees_win()
        # END Problem 13
...
def make_slow(action, bee):
    ...
    """
    # BEGIN Problem EC
    def bee_new_action(colony):
        if colony.time % 2 == 0:
            action(colony)
    return bee_new_action
    # END Problem EC
...
def make_scare(action, bee):
    ...
    def bee_new_action(colony):
        ...
        # print("DEBUG: dest:", destination)
        # print("DEBUG: blocked:", bee.blocked())
        if bee.blocked():
            bee.sting(bee.place.ant)
        elif bee.armor > 0 and destination is not colony.beehive:
...
def apply_effect(effect, bee, duration):
    ...
    def action(colony):
        ...
        # print("DEBUG: location:", bee.place.name)
        if duration <= 0:
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        if target and not target.is_scared:
            apply_effect(make_scare, target, self.duration)
